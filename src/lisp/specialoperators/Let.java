...
public class Let extends SpecialOperator{
	...
	private HashMap<String, Integer> createInitMap(List list) throws SyntaxException{
		...
		for(LispForm it : list.getChildren()){
			...
			res.put(varName.getName(), ((Int) ((List) it).getChildren().get(1)).getValue());
		}
		return res;	
	}
	...
	public void compile(SymbolTable symbolTable) throws SyntaxException{
		...
		HashMap<String, Integer> initMap = createInitMap((List) parameters.get(0));
	    ...
	    while (it.hasNext()) {
	        ...
	        Factory.getMethodVisitor().visitLdcInsn(pairs.getValue());
		...
		while (it.hasNext()) {
	        ...
	        Factory.getMethodVisitor().visitLdcInsn(newSymbolTable.getAddress(pairs.getKey()));
			Factory.getMethodVisitor().visitMethodInsn(Opcodes.INVOKESTATIC, "lisp/RT/MemoryPool", "pop", "(I)I");
	    }
	}
}
